defmodule Day03 do
  ...
  #   left = "R8,U5,L5,D3"
  #   right = "U7,R6,D4,L4"
  #   left = parse_data(left)
  #   right = parse_data(right)
  ...
  def part2() do
    ...
    crossings(left, right)
    |> shortest_wire(left, right)
    |> path_lengths(left, right)
  end
  ...
  def path_length(data, crossing) do
    data
    |> Enum.with_index(1)
    |> Enum.find(fn {pos, _index} -> pos == crossing end)
    |> elem(1)
  end

  def path_lengths(crossing, left, right) do
    path_length(left, crossing) + path_length(right, crossing)
  end

  def shortest_wire(crossings, left, right) do
    Enum.min_by(crossings, fn crossing ->
      path_lengths(crossing, left, right)
    end)
  end

  def shortest_distance(data) do
    Enum.min_by(data, fn pos -> manhattan_distance(pos) end)
  end

  def crossings(left, right) do
  ...
  def parse_data(data) do
    data
    |> String.split(",")
    |> map_parse_dir(@start_pos)
    |> map_extrapolate(@start_pos)
  end

  def map_extrapolate(positions, start_pos) do
    Enum.flat_map_reduce(positions, start_pos, fn pos, acc ->
      {extrapolate(acc, pos), pos}
    end)
    |> elem(0)
  end

  def extrapolate({x1, y1}, {x2, y2}) do
    for(x <- x1..x2, y <- y1..y2, do: {x, y})
    |> Kernel.tl()
  end

  def map_parse_dir(directions, start_pos) do
    directions
    |> Enum.map_reduce(start_pos, fn dir, acc ->
      acc = parse_dir(dir, acc)
      {acc, acc}
    end)
    |> elem(0)
  end

  def parse_dir(<<dir>> <> len, pos) do
    len = String.to_integer(len)
    parse_dir(<<dir>>, len, pos)
  end

  def parse_dir("U", len, {x, y}),
    do: {x, y + len}

  def parse_dir("D", len, {x, y}),
    do: {x, y - len}

  def parse_dir("L", len, {x, y}),
    do: {x - len, y}

  def parse_dir("R", len, {x, y}),
    do: {x + len, y}
end
